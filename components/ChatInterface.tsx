...
import { Send, Loader2, MoreVertical, Phone, Video, BadgeCheck, CheckCheck, Mic, Square } from 'lucide-react';
...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({ messages, onSendMessage, isLoading }) => {
  ...
  useEffect(() => {
    ...
    if (SpeechRecognition) {
      ...
      // MODE: Berhenti otomatis saat user diam (Silence Detection)
      recognitionRef.current.continuous = false; 
      
      // MODE: Jangan munculkan teks real-time (Hanya hasil akhir)
      // Ini mencegah bug "Saya saya saya tidak..."
      recognitionRef.current.interimResults = false;
      
      recognitionRef.current.lang = 'id-ID';

      recognitionRef.current.onresult = (event: any) => {
        // Ambil hanya hasil final yang sudah diproses browser secara utuh
        const transcript = event.results[0][0].transcript;
        
        const base = initialTextRef.current.trim();
        
        // Gabungkan secara bersih
        const combined = base 
          ? `${base} ${transcript}` 
          : transcript;
  ...
  return (
    <div className="flex flex-col h-full bg-[#efe7dd] relative font-sans">
      {/* Header */}
      <div className="h-16 bg-[#008069] flex items-center px-4 shrink-0 z-10 shadow-md text-white">
        <div className="flex items-center flex-1 gap-3">
          ...
          <div className="flex flex-col justify-center">
            ...
            <span className="text-[11px] opacity-90 text-white/90">Asisten Virtual JKN</span>
          </div>
        </div>
      ...
      <div className="p-2 bg-[#f0f2f5] flex items-center gap-2 shrink-0 pb-safe">
        <div className="flex-1 bg-white rounded-full border-none px-4 py-2 flex items-center shadow-sm gap-2">
          ...
          <button 
            ...
            title={isListening ? "Berhenti Merekam" : "Mulai Merekam"}
          >
            {isListening ? (
